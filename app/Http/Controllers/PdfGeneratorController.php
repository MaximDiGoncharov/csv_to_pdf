...
//        $templatesDir = storage_path('app/templates');
        $templatesDir = storage_path('app/templates_html');
...
//            $templatePath = 'templates/' . $request->template;
            $templatePath = 'templates_html/' . $request->template;
...
//            $templatePath = 'templates/' . $request->template;
    ...
    public function generatePdf(Request $request)
    {
        ...
        $startRow = session('export_start_row', 1);
        $rowCount = session('export_row_count', 250);
        ...
        try {
            $csvData = array_slice($csvData, $startRow, $rowCount);
            $filename = $this->pdfService->generatePdfFromHtmlTemplate($pdfPath, $csvData, $fieldMapping);
    ...
    public function saveExportSettings(Request $request)
    {
        $request->validate([
            'start_row' => 'required|integer|min:1',
            'row_count' => 'required|integer|min:1|max:500'
        ]);

        session([
            'export_start_row' => $request->start_row,
            'export_row_count' => $request->row_count
        ]);
