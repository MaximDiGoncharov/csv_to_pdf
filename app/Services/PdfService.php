...
class PdfService
{
     ...
     */
    public function _generate(string $templatePath, array $csvData, array $fieldMapping): string
    {
    ...
    /**
     * Генерирует PDF из HTML шаблона с переменными
     */
    public function generatePdfFromHtmlTemplate($templatePath, $csvData, $fieldMapping)
    {

        $templateContent = file_get_contents(base_path() . '/storage/app/' . $templatePath);
        if ($templateContent === false) {
            throw new \Exception('Не удалось прочитать шаблон HTML');
        }
        $result = '';
        $count = 0;

        // Подготовка один раз перед циклом
        $searchPatterns = array_map(fn($field) => '{{' . $field . '}}', array_keys($fieldMapping));

        $result = '';
        foreach ($csvData as $row) {

            $replaceValues = array_map(fn($csvField) => $row[$csvField], $fieldMapping);
            $result .= str_replace($searchPatterns, $replaceValues, $templateContent);
        }
        $templateContent = $result;

        // Инициализация Dompdf
        $dompdf = new Dompdf();
        $dompdf->loadHtml($templateContent);
        ...
        // Сохраняем или выводим файл
        $outputPath = storage_path('app/generated/' . 'pdf_' . time() . '.pdf');
        file_put_contents($outputPath, $dompdf->output());

        return $outputPath; // Возвращает путь к сгенерированному PDF
    }
